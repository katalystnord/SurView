...
{
    // The ramp itself lives in gui/FieldColours.h, so every panel that draws a
    // field -- this viewport and the comparison screen's three -- draws it in
    // the same colours.
    buildFieldColourTable(m_fieldColours, diverging,
                          fieldChannelIsFlag(m_fieldChannel), lowest, highest);
}
